...
class MyGLSurfaceView extends GLSurfaceView {
	...
	private class SceneRenderer implements GLSurfaceView.Renderer {
		MyPyramid[] pArray;// 金字塔数组
		MyDesert desert;// 沙漠
		MyCelestial celestialSmall;// 小星星星空半球
		MyCelestial celestialBig;// 大星星星空半球

		float lightAngle = 120.0f;// 阳光的角度
		float allAngle = 0f;

		public SceneRenderer() {
			new Thread() {// 定时旋转阳光的线程
				public void run() {
					while (true) {
						lightAngle += 0.5;
						allAngle += 0.15;
		...
		public void onDrawFrame(GL10 gl) {
			...
			float[] positionParamsGreen = { lxSun, lySun, 0f, 0 };// 最后的0表示使用定向光
			...
			// gl.glRotatef(allAngle, 0, 1f, 0);
			gl.glPushMatrix();
			// 绘制金字塔
			for (MyPyramid tp : pArray) {
				tp.drawSelf(gl);
			}
			// 绘制沙漠
			gl.glPopMatrix();
		...
		public void onSurfaceCreated(GL10 gl, EGLConfig config) {
			...
			// 创建金字塔
			pArray = new MyPyramid[] {
					new MyPyramid(-2, -2, 2.0f, 30, wallaId),
					new MyPyramid(3, -7, 2.0f, 50, wallbId),
					new MyPyramid(6, -2, 2.0f, 0, wallcId), };
			// 创建沙漠
			desert = new MyDesert(-20, -20, 4, 0, desertId, 40, 40);
			// 创建星空
			celestialSmall = new MyCelestial(0, 0, 1f, 0f, 250);
			celestialBig = new MyCelestial(0, 0, 2f, 0f, 50);
	...
	public void initFog(GL10 gl) {
		...
		gl.glFogf(GL10.GL_FOG_DENSITY, 0.5f);// 设置雾的浓度，浓度范围为0~1之间，0表示最低浓度，1表示最高浓度。
