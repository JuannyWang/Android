 ...
 * 
 */
package ghost.android3d.opengl40;
 ...
 * @date 2013-2-3
 */
public class MyBallControl implements Constants {
	...
	public MyBallControl(MyBall btv, float highl) {
		this.btv = btv;
		this.high = highl;
		currentY = highl;
			...
			public void run() {
				while (true) {
					if (isDown) {// 下降阶段
						...
						if (vyMax < 0.35f) {// 若速度小于阈值则停止运动
							currentY = 0;// 球位移置0
							break;
						}

						if (currentVy > 0) {// 若速度大于零则继续上升
							currentY = vyMax * timeLive - 0.5f * G * timeLive
									* timeLive;// 上升位移
							timeLive += TIME_SPAN;// 运动时间
							currentVy = vyMax - G * timeLive;// 当前速度
						} else {// 若速度小于等于0则进入下降周期
							timeLive = 0;// 时间置0
	...
	public void drawSelf(GL10 gl) {// 绘制物体自己
		gl.glPushMatrix();
	...
	public void drawSelfMirror(GL10 gl) {// 绘制 镜像体
		gl.glPushMatrix();
		gl.glTranslatef(0, -UNIT_SIZE * BALL_SCALE - currentY, 0);
		btv.drawSelf(gl);
		gl.glPopMatrix();
