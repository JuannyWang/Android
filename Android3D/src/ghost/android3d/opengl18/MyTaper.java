 ...
 * 
 */
package ghost.android3d.opengl18;
...
public class MyTaper {
	...
	public MyTaper(float height, float circle_radius, float degreespan,
		...
		float normalHeight = (height * circle_radius * circle_radius)
				/ (height * height + circle_radius * circle_radius);// 法向量点所在圆截面的高度
		float normalR = (height * height * circle_radius)
				/ (height * height + circle_radius * circle_radius);// 法向量点所在圆截面的半径
		...
		for (float circle_degree = 360.0f; circle_degree > 0.0f; circle_degree -= degreespan)// 循环行
		{
			for (int j = 0; j < col; j++)// 循环列
			{
				...
				float c1 = (float) (normalR * Math.sin(Math
						.toRadians(circle_degree)));
				...
				float c2 = (float) (normalR * Math.sin(Math
						.toRadians(circle_degree)));
				...
				float c3 = (float) (normalR * Math.sin(Math
						.toRadians(circle_degree - degreespan)));
				...
				float c4 = (float) (normalR * Math.sin(Math
						.toRadians(circle_degree - degreespan)));
