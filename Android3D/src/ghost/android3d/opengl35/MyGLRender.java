...
public class MyGLRender implements Renderer {
	...
	MyFullTextureBall earthCloud;// 地球云层
	...
	private int haloTextureId;// 纹理名称ID
	private int cloudTextureId;// 纹理名称ID
	...
	public void onDrawFrame(GL10 gl) {
		...
		// 设置源混合因子与目标混合因子
		gl.glBlendFunc(GL10.GL_SRC_ALPHA,GL10.GL_ONE); // 混合效果设置
		...
		// 绘制光晕
		gl.glEnable(GL10.GL_BLEND);
		gl.glBlendFunc(GL10.GL_SRC_ALPHA, GL10.GL_ONE_MINUS_SRC_ALPHA); // 混合效果设置
		halo.drawSelf(gl);
		gl.glDisable(GL10.GL_BLEND);
	}
	...
	public void onSurfaceCreated(GL10 gl, EGLConfig config) {
		...
		earthCloud = new MyFullTextureBall(6.1f, cloudTextureId);

		// 创建地球光晕
		halo = new MyTextureRect(haloTextureId);
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				while (true) {
					...
					earthCloud.mAngleY += TOUCH_SCALE_FACTOR;
					try {
