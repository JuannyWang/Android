 ...
 * @date 2013-1-27
 */
public class MyVertexCube {
	private FloatBuffer mVertexBuffer;//顶点坐标数据缓冲
	private FloatBuffer mNormalBuffer;//法向量数据缓冲
	public float mOffsetX;// 绕x轴旋转
	public float mOffsetY;//绕Y轴旋转
	float height;						//立方体高度			
	int vCount;//顶点数量
	public MyVertexCube(float height,float length,float width)
	{
		this.height=height;
		vCount=36;
		...
		float[] verteices=
		{
				...
				//顶面
				-UNIT_SIZE*length,UNIT_HIGHT*height,-UNIT_SIZE*width,
				-UNIT_SIZE*length,UNIT_HIGHT*height,UNIT_SIZE*width,
				UNIT_SIZE*length,UNIT_HIGHT*height,-UNIT_SIZE*width,
