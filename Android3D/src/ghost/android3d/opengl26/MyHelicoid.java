 ...
 * 
 */
package ghost.android3d.opengl26;
...
public class MyHelicoid {
	...
	public MyHelicoid(float circle_small_R, // 截面半径
		...
		for (float h_angle = 0, c_r = circle_small_R, h_r = Hedicoid_small_R, length = 0; h_angle < MAX_ANGLE; h_angle += HEDICOID_ANGLE_SPAN, c_r += CIRCLE_R_SPAN, h_r += HEDICOID_R_SPAN, length += LENGTH_SPAN) {
			for (float c_angle = CIECLE_ANGLE_BEGIN; c_angle < CIECLE_ANGLE_OVER; c_angle += CIRCLE_ANGLE_SPAN) {
				float x1 = (float) ((h_r + c_r
						* Math.cos(Math.toRadians(c_angle))) * Math.cos(Math
						.toRadians(h_angle)));
				float y1 = (float) (c_r * Math.sin(Math.toRadians(c_angle)) + length);
				float z1 = (float) ((h_r + c_r
						* Math.cos(Math.toRadians(c_angle))) * Math.sin(Math
						.toRadians(h_angle)));

				float x2 = (float) ((h_r + c_r
						* Math.cos(Math.toRadians(c_angle + CIRCLE_ANGLE_SPAN))) * Math
						.cos(Math.toRadians(h_angle)));
				float y2 = (float) (c_r
						* Math.sin(Math.toRadians(c_angle + CIRCLE_ANGLE_SPAN)) + length);
				float z2 = (float) ((h_r + c_r
						* Math.cos(Math.toRadians(c_angle + CIRCLE_ANGLE_SPAN))) * Math
						.sin(Math.toRadians(h_angle)));

				float x3 = (float) (((h_r + HEDICOID_R_SPAN) + (c_r + CIRCLE_R_SPAN)
						* Math.cos(Math.toRadians(c_angle + CIRCLE_ANGLE_SPAN))) * Math
						.cos(Math.toRadians(h_angle + HEDICOID_ANGLE_SPAN)));
				float y3 = (float) ((c_r + CIRCLE_R_SPAN)
						* Math.sin(Math.toRadians(c_angle + CIRCLE_ANGLE_SPAN)) + (length + LENGTH_SPAN));
				float z3 = (float) (((h_r + HEDICOID_R_SPAN) + (c_r + CIRCLE_R_SPAN)
						* Math.cos(Math.toRadians(c_angle + CIRCLE_ANGLE_SPAN))) * Math
						.sin(Math.toRadians(h_angle + HEDICOID_ANGLE_SPAN)));

				float x4 = (float) (((h_r + HEDICOID_R_SPAN) + (c_r + CIRCLE_R_SPAN)
						* Math.cos(Math.toRadians(c_angle))) * Math.cos(Math
						.toRadians(h_angle + HEDICOID_ANGLE_SPAN)));
				float y4 = (float) ((c_r + CIRCLE_R_SPAN)
						* Math.sin(Math.toRadians(c_angle)) + (length + LENGTH_SPAN));
				float z4 = (float) (((h_r + HEDICOID_R_SPAN) + (c_r + CIRCLE_R_SPAN)
						* Math.cos(Math.toRadians(c_angle))) * Math.sin(Math
						.toRadians(h_angle + HEDICOID_ANGLE_SPAN)));
				...
				float a1 = (float) (x1 - (h_r * Math.cos(Math
						.toRadians(h_angle))));
				float b1 = y1 - length;
				float c1 = (float) (z1 - (h_r * Math.sin(Math
						.toRadians(h_angle))));
				...
				float a2 = (float) (x2 - (h_r * Math.cos(Math
						.toRadians(h_angle))));
				float b2 = y1 - length;
				float c2 = (float) (z2 - (h_r * Math.sin(Math
						.toRadians(h_angle))));
				...
				float a3 = (float) (x3 - (h_r * Math.cos(Math.toRadians(h_angle
						+ HEDICOID_ANGLE_SPAN))));
				float b3 = y1 - (length + LENGTH_SPAN);
				float c3 = (float) (z3 - (h_r * Math.sin(Math.toRadians(h_angle
						+ HEDICOID_ANGLE_SPAN))));
				...
				float a4 = (float) (x4 - (h_r * Math.cos(Math.toRadians(h_angle
						+ HEDICOID_ANGLE_SPAN))));
				float b4 = y1 - (length + LENGTH_SPAN);
				float c4 = (float) (z4 - (h_r * Math.sin(Math.toRadians(h_angle
						+ HEDICOID_ANGLE_SPAN))));
