...
public class MyWater {
	...
	public MyWater(int texId, double startAngle, int rows, int cols, int bei) {
		this.texId = texId;
		final double angleSpan = Math.PI * 4 / cols;// 每格的单位弧度
		final float LOCAL_UNIT_SIZE = bei * Constants.UNIT_SIZE;
		...
		for (int j = 0; j < rows; j++) {
			for (int i = 0; i < cols; i++) {
				...
				float zsy = (float) Math.sin(startAngle + i * angleSpan) * 0.4f
						- Constants.WATER_HIGH_ADJUST;
				// 计算右边一列的y坐标
				float zsyp = (float) Math.sin(startAngle + (i + 1) * angleSpan)
						* 0.4f - Constants.WATER_HIGH_ADJUST;
