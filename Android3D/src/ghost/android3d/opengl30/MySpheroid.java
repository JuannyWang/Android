 ...
 * 
 */
package ghost.android3d.opengl30;
...
public class MySpheroid {
	...
	public MySpheroid(int scale) {
		...
		for (int vAngle = -90; vAngle <= 90; vAngle = vAngle + angleSpan)// 垂直方向angleSpan度一份
		{
			...
			{// 纵向横向各到一个角度后计算对应的此点在球面上的坐标
				int x = (int) (a * scale * UNIT_SIZE
						* Math.cos(Math.toRadians(vAngle)) * Math.cos(Math
						.toRadians(hAngle)));
				int y = (int) (b * scale * UNIT_SIZE
						* Math.cos(Math.toRadians(vAngle)) * Math.sin(Math
						.toRadians(hAngle)));
				int z = (int) (c * scale * UNIT_SIZE * Math.sin(Math
						.toRadians(vAngle)));
		...
		for (int i = 0; i < vCount; i++) {// 随机生成每个顶点的颜色
		...
		{
			if (i > 0 && i < row - 1) {// 中间行
				for (int j = -1; j < col; j++) {// 中间行的两个相邻点与下一行的对应点构成三角形
					int k = i * col + j;
					alIndex.add(k + col);
					alIndex.add(k + 1);
					alIndex.add(k);
	...
	public void drawSelf(GL10 gl) {
		...
		// 绘制图形
		gl.glDrawElements(GL10.GL_TRIANGLES, // 以三角形方式填充
