 ...
 * 
 */
package ghost.android3d.opengl23;
...
public class MyLineParaboloid {
	...
	public MyLineParaboloid(float height, float a, float b, int col,
			float angleSpan) {
		this.height = height;
		this.a = a;
		this.b = b;
		...
		for (float h = height; h > 0; h -= heightSpan)// 循环行
		{
			for (float angle = 360; angle > 0; angle -= angleSpan)// 循环列
			{
				...
				float x3 = (float) (a * Math.sqrt(2 * (h - heightSpan)) * Math
						.cos(Math.toRadians(angle - angleSpan)));
				float y3 = h - heightSpan;
				float z3 = (float) (b * Math.sqrt(2 * (h - heightSpan)) * Math
						.sin(Math.toRadians(angle - angleSpan)));
