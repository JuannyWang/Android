...
public class EnLrcView extends TextView{
	private float width;
	private float high;
	...
	public EnLrcView(Context context) {
		super(context);
		init();
		// TODO Auto-generated constructor stub
	}

	public EnLrcView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
		// TODO Auto-generated constructor stub
	}
	public EnLrcView(Context context, AttributeSet attrs, int defStyle) {
		...
		try {
			if (MusicActivity.showLrc.isChecked()) {
				...
				for (int i = Index; i >=0;i--) {//画出本句及之前的句子
					if (i==Index) {
						tempY=DrawLrcIndex(canvas, CurrentPaint, mEnSentenceEntities.get(i).getLrc(), tempY);
						...
						for(int j=Index+1;j<mEnSentenceEntities.size();j++)
						{
							...
							temp=DrawLrcDownward(canvas, NotCurrentPaint, mEnSentenceEntities.get(j).getLrc(), temp);
						}
					}
					else{
						...
						tempY=DrawLrcUpward(canvas, NotCurrentPaint,mEnSentenceEntities.get(i).getLrc(), tempY);
					}
				}
